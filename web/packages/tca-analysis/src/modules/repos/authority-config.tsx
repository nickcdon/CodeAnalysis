...
const RepoAuthInfo = ({ scmAuth }: RepoAuthInfoProps) => {
  ...
  let txt = '';
  if (authType === AUTH_TYPE.OAUTH && scmOAuth) {
    txt = `${AUTH_TYPE_TXT.OAUTH}: ${scmOAuth.user} (由 ${scmOAuth.user?.nickname} 在 ${scmOAuth.auth_origin} 创建)`;
  } else if (authType === AUTH_TYPE.SSH && scmSSH) {
    txt = `${AUTH_TYPE_TXT.SSH}: ${scmSSH.name} (由 ${scmSSH.user?.nickname} 在 ${scmSSH.auth_origin} 创建)`;
  } else if (authType === AUTH_TYPE.HTTP && scmAccount) {
    txt = `${AUTH_TYPE_TXT.HTTP}: ${scmAccount.scm_username} (由 ${scmAccount.user?.nickname} 在 ${scmAccount.auth_origin} 创建)`;
  }
  if (txt === '') {
...
const AuthorityConfig = ({ orgSid, teamName, repoInfo, visible, onCancel, callback }: AuthorityConfigProps) => {
  ...
  const onFinish = ({ scm }: any) => {
    if (scm) {
      ...
      const scmAuth = SCM_MAP[authType];
